...
public class BufferPool {
	...
	public Page getPage(TransactionId tid, PageId pid, Permissions perm) throws TransactionAbortedException,
			DbException {
		// some code goes here
		Page pa=pages.get(pid);
		if(pa != null)
			return pa;
		else{
			if(pages.size() >= numPages)
		throw new DbException("more pages than size");
			Page new_pa = Database.getCatalog().getDbFile(pid.getTableId()).readPage(pid);
			pages.put(pid, new_pa);
			return  new_pa;
		}
