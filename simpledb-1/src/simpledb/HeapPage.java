...
public class HeapPage implements Page {
	...
	public Iterator<Tuple> iterator() {
		...
		ArrayList <Tuple> temp_tuple_list = new ArrayList<Tuple>();
		
		     for(int i=0; i<entryCount();i++)
		     { 	 
		     if(getTuple(i)!=null)
		    	 temp_tuple_list.add(getTuple(i));
		     }
          
		     return temp_tuple_list.iterator();
	}
	...
	public Tuple getTuple(int entryID) {
		...
		// some code goes here
		int entry_location = tupleLocation(entryID);
		int entry_count = entryCount();
		if( entryID < entry_location)
		{
			DataInputStream in	=new DataInputStream(new ByteArrayInputStream(data,entry_location, data.length- entry_location));
			Tuple create_tuple = createTuple(in);
			create_tuple.setRecordId(new RecordId(pid, entryID));
			return create_tuple;
		}
		else
		return null;
